...
function getTotalRecovered(data) {
    ...
    for (var i = 0; i != data.length; i++) {
        ...
        countries.forEach(item => {
            if (item.Country_Region === data[i]['attributes']['Country_Region']) {
                item.Recovered = parseInt(item.Recovered) + parseInt(data[i]['attributes']['Recovered']);
                tmp = true;
            }
        });
    ...
    for (var j = 0; j < countries.length; j++) {
        countries[j].Recovered = numberWithSpaces(countries[j].Recovered);
    }
    return countries;
}

export class Recovered extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: null,
    ...
    componentDidMount() {
        ...
        fetch('https://services1.arcgis.com/0MSEUqKaxRlEPj5g/ArcGIS/rest/services/Coronavirus_2019_nCoV_Cases/FeatureServer/1/query' + query)
            ...
            .then(data => {
                ...
                this.setState({
                    cases: getTotalRecovered(this.state.data),
                });
            });
    }
            ...
            mylist.push(
                <View style={styles} key={i}>
                    <Text style={styles.line}>
                        <Text style={[styles.subtitle, styles.red_color]}>{this.state.cases[i]['Recovered']} </Text>
                        <Text> </Text>
