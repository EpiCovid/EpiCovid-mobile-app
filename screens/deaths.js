...
function getTotalDeaths(data) {
    ...
    for (var i = 0; i != data.length; i++) {
        ...
        countries.forEach(item => {
            if (item.Country_Region === data[i]['attributes']['Country_Region']) {
                item.Deaths = parseInt(item.Deaths) + parseInt(data[i]['attributes']['Deaths']);
                tmp = true;
            }
        });
    ...
    for (var j = 0; j < countries.length; j++) {
        countries[j].Deaths = numberWithSpaces(countries[j].Deaths);
    }
    return countries;
}
    ...
    componentDidMount() {
        ...
        fetch('https://services1.arcgis.com/0MSEUqKaxRlEPj5g/ArcGIS/rest/services/Coronavirus_2019_nCoV_Cases/FeatureServer/1/query' + query)
            ...
            .then(data => {
                ...
                this.setState({
                    cases: getTotalDeaths(this.state.data),
                });
            });
    }
            ...
            mylist.push(
                <View style={styles} key={i}>
                    <Text style={styles.line}>
                        <Text style={[styles.subtitle, styles.red_color]}>{this.state.cases[i]['Deaths']} </Text>
                        <Text> </Text>
